...
class CImageHandler extends CApplicationComponent
{
    ...
    public function resizeCrop($toWidth, $toHeight)
    {
        $this->checkLoaded();
        $cropWidth = round($this->width * $toHeight / $this->height);
        $cropHeight = round($this->height * $toWidth / $this->width);

        if($cropWidth > $toWidth)
        {
            $newWidth = $cropWidth;
            $newHeight = $toHeight;
        }
    ...
    public function save($file = false, $toFormat = false, $jpegQuality = 95, $touch = false)
    {
        if (empty($file))
        {
        	$file = $this->fileName;
